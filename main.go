...
func initGame(database *sql.DB, cfg config.Config) (err error) {
	...
	gameStart := cfg.Game.Start.Time
	gameEnd := cfg.Game.End.Time
	if v, serr := db.GetSetting(database, "game_start"); serr == nil && v != "" {
		if t, perr := time.Parse(time.RFC3339, v); perr == nil {
			gameStart = t
			log.Println("Game start overridden from admin panel")
		}
	}
	if v, serr := db.GetSetting(database, "game_end"); serr == nil && v != "" {
		if t, perr := time.Parse(time.RFC3339, v); perr == nil {
			gameEnd = t
			log.Println("Game end overridden from admin panel")
		}
	}

	log.Println("Start game at", gameStart)
	log.Println("End game at", gameEnd)
	g, err := game.NewGame(database, gameStart, gameEnd, teamBase)
	if err != nil {
		return
	}
	...
	if cfg.Admin.Enabled {
		scoreboard.EnableAdmin(cfg.Admin.Token)
	}
...
func main() {
	...
	scoreboard.SupportLog = filepath.Join(filepath.Dir(cfg.LogFile),
		"support.log")
	scoreboard.SupportFiles = filepath.Join(filepath.Dir(cfg.LogFile),
		"support-files")
